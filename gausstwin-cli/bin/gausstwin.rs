...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Start { config, env, port, debug } => {
            // Load configuration
        ...
        Commands::Backup { config, output, name, compress } => {
        ...
        Commands::Restore { backup, config, yes } => {
            ...
            println!("\n✅ Database restored successfully from: {}", backup.display());
            
            Ok(())
        }
        
        Commands::Benchmark { benchmark_type, iterations, agents, output } => {
            ...
            results.insert("version".to_string(), serde_json::json!(env!("CARGO_PKG_VERSION")));
            results.insert("timestamp".to_string(), serde_json::json!(chrono::Utc::now().to_rfc3339()));
            ...
            if benchmark_type == "all" || benchmark_type == "agents" {
                ...
                results.insert("agents".to_string(), serde_json::json!({
            ...
            if benchmark_type == "all" || benchmark_type == "spaces" {
                ...
                for _ in 0..iterations {
                    ...
                    let _dist = ((rand::random::<f64>() - 0.5).powi(2) + 
                                (rand::random::<f64>() - 0.5).powi(2)).sqrt();
                    pb.inc(1);
                }
                let duration = start.elapsed();
                ...
                let ops_per_sec = iterations as f64 / duration.as_secs_f64();
                results.insert("spaces".to_string(), serde_json::json!({
                    "iterations": iterations,
                    "duration_ms": duration.as_millis(),
                    "ops_per_sec": ops_per_sec,
                }));
            ...
            if benchmark_type == "all" || benchmark_type == "events" {
                ...
                let ops_per_sec = iterations as f64 / duration.as_secs_f64();
                results.insert("events".to_string(), serde_json::json!({
                    "iterations": iterations,
                    "duration_ms": duration.as_millis(),
                    "ops_per_sec": ops_per_sec,
                }));
        ...
        Commands::Export { simulation, output, format, url } => {
