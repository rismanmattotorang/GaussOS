...
use gausstwin_data::{
    UnifiedStore, UnifiedStoreConfig, VectorStoreConfig, DbConfig,
    CacheConfig, PoolConfig, MetricsConfig, VectorData, ScalarData,
    Value, QueryFilters, FilterOperator, FilterCondition, HybridRecord,
...
fn create_test_scalar_data() -> ScalarData {
    ...
    fields.insert("string_field".to_string(), Value::String("test".to_string()));
...
async fn test_basic_operations() {
    ...
    let id = store.store_hybrid("test_key", &vector_data, &scalar_data).await.unwrap();
    assert!(!id.is_nil());
...
async fn test_batch_operations() {
    ...
    for i in 0..10 {
        ...
        scalar_data.fields.insert("index".to_string(), Value::Integer(i));
...
async fn test_hybrid_search() {
    ...
    for i in 0..100 {
        ...
        store.store_hybrid(&format!("test_key_{}", i), &vector_data, &scalar_data).await.unwrap();
    }
    ...
    // Test search
    let results = store.hybrid_search(&query_vector.as_slice().unwrap(), &scalar_filters, 10).await.unwrap();
...
async fn test_streaming_search() {
    ...
    let mut stream = store.stream_hybrid_search(&query_vector.as_slice().unwrap(), &scalar_filters, 10).await.unwrap();
...
async fn test_cache_behavior() {
    ...
    // Store data
    store.store_hybrid("cache_test", &vector_data, &scalar_data).await.unwrap();
...
async fn test_error_handling() {
    ...
    let result = store.store_hybrid("error_test", &invalid_vector_data, &scalar_data).await;
...
async fn test_concurrent_operations() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            let vector_data = create_test_vector_data(128);
            let mut scalar_data = create_test_scalar_data();
            scalar_data.fields.insert("thread_id".to_string(), Value::Integer(i));
            
            let key = format!("concurrent_test_{}", i);
            let id = store_clone.store_hybrid(&key, &vector_data, &scalar_data).await.unwrap();
