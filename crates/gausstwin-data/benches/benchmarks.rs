...
fn create_test_scalar_data() -> ScalarData {
    ...
    fields.insert("string_field".to_string(), Value::String("bench".to_string()));
...
fn bench_batch_store(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("batch_size", size), &records, |b, records| {
            b.iter(|| {
                rt.block_on(async {
...
fn bench_get_hybrid(c: &mut Criterion) {
    ...
    rt.block_on(async {
        store.store_hybrid("bench_get", &vector_data, &scalar_data).await.unwrap();
    });
    ...
    group.bench_function("cache_miss", |b| {
        b.iter(|| {
            rt.block_on(async {
                store.get_hybrid(black_box("bench_get")).await.unwrap()
            })
        })
    });
...
fn bench_hybrid_search(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..1000 {
            ...
            store.store_hybrid(&format!("bench_search_{}", i), &vector_data, &scalar_data)
                .await
                .unwrap();
        }
    });
...
fn bench_streaming_search(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..1000 {
            ...
            store.store_hybrid(&format!("bench_stream_{}", i), &vector_data, &scalar_data)
                .await
                .unwrap();
        }
    });
    ...
    for batch_size in [10, 50, 100].iter() {
        group.bench_with_input(BenchmarkId::new("batch_size", batch_size), batch_size, |b, &batch_size| {
            b.iter(|| {
                rt.block_on(async {
                    let mut stream = store.stream_hybrid_search(
                        black_box(&query_vector.as_slice().unwrap()),
                        black_box(&scalar_filters),
                    ...
                    rt.block_on(async {
                        ...
                        for i in 0..num_concurrent {
                            ...
                            let handle = tokio::spawn(async move {
                                ...
                                let id = store_clone.store_hybrid(&key, &vector_data, &scalar_data)
                                    .await
                                    .unwrap();
