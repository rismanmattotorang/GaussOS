...
impl GrpcServer {
    ...
    fn error_to_status(error: Error) -> Status {
        match error {
            ...
            Error::Authorization(msg) | Error::PermissionDenied(msg) => Status::permission_denied(msg),
...
impl TwinService for GrpcServer {
    async fn get_twin(
        ...
        let twin = self.state.db.get_twin(&req.id).await
            .map_err(Self::error_to_status)?
            .ok_or_else(|| Status::not_found(format!("Twin {} not found", req.id)))?;
        
        self.state.metrics.increment_counter("grpc.get_twin.calls", 1, None);
    ...
    async fn create_twin(
        ...
        let twin = self.state.db.create_twin(crate::graphql::CreateTwinInput {
            name: req.name,
            description: Some(req.description),
        }).await.map_err(Self::error_to_status)?;
        
        self.state.metrics.increment_counter("grpc.create_twin.calls", 1, None);
    ...
    async fn update_twin(
        ...
        let twin = self.state.db.update_twin(crate::graphql::UpdateTwinInput {
        ...
        self.state.metrics.increment_counter("grpc.update_twin.calls", 1, None);
    ...
    async fn delete_twin(
        ...
        let success = self.state.db.delete_twin(&req.id).await
            .map_err(Self::error_to_status)?;
        
        self.state.metrics.increment_counter("grpc.delete_twin.calls", 1, None);
    ...
    async fn list_twins(
        ...
        let twins = self.state.db.list_twins(limit, offset, None).await
            .map_err(Self::error_to_status)?;
        
        let twin_responses: Vec<TwinResponse> = twins.into_iter()
            .map(|t| TwinResponse {
                id: t.id,
                name: t.name,
        ...
        self.state.metrics.increment_counter("grpc.list_twins.calls", 1, None);
    ...
    async fn start_simulation(
        ...
        let result = self.state.db.start_simulation(&req.twin_id).await
            .map_err(Self::error_to_status)?;
        
        self.state.metrics.increment_counter("grpc.start_simulation.calls", 1, None);
        
        Ok(Response::new(SimulationResponse {
            id: result.id.to_string(),
            twin_id: result.twin_id.to_string(),
            status: result.status,
            results: result.metrics.into_iter().map(|m| MetricValue {
                name: m.name,
                value: m.value,
    ...
    async fn stream_metrics(
        ...
        let stream = tokio_stream::wrappers::BroadcastStream::new(rx)
            .filter_map(move |result| {
                match result {
                    Ok(update) if update.twin_id == twin_id && 
        ...
        self.state.metrics.increment_counter("grpc.stream_metrics.calls", 1, None);
        
        Ok(Response::new(Box::pin(stream) as Self::StreamMetricsStream))
    }
    
    type BidirectionalStreamStream = Pin<Box<dyn Stream<Item = Result<MetricUpdate, Status>> + Send>>;
    ...
    async fn bidirectional_stream(
        ...
        self.state.metrics.increment_counter("grpc.bidirectional_stream.calls", 1, None);
        
        Ok(Response::new(Box::pin(stream) as Self::BidirectionalStreamStream))
    }
