    ...
    async fn act(&mut self, action: &Self::Action, ctx: &mut AgentContext) -> Result<(), AgentError> {
        ...
        // Update metrics
        {
            ...
            metrics.ops_per_second = 1.0 / (
                metrics.avg_sensor_time +
                metrics.avg_behavior_time +
...
impl ReactiveAgent {
    ...
    fn compute_behaviors_simd(
        ...
        let (mut avoidance, mut cohesion, mut alignment): (Vec<f64>, Vec<f64>, Vec<f64>) = (Vec::new(), Vec::new(), Vec::new());
        for chunk in chunks {
            let simd = F64x4::from_slice_unaligned(chunk);
            // Compute separation
    ...
    fn combine_behaviors_simd(
        ...
        let len = avoidance_slice.len().min(cohesion_slice.len()).min(alignment_slice.len());
        ...
        for i in (0..len).step_by(4) {
            ...
            let combined_simd =
                av_simd * F64x4::splat(weights.avoidance) +
    ...
    async fn update_position_simd(&mut self, ctx: &mut AgentContext) -> Result<(), AgentError> {
        ...
        let neighbors = ctx.space.get_neighbors(self.id, self.sensors.ranges.get("sensor_range").copied().unwrap_or(1.0)).await?;
    ...
    fn compute_flocking_behavior_simd(&self, avoidance: &Array1<f64>, cohesion: &Array1<f64>, alignment: &Array1<f64>) -> Array1<f64> {
        ...
        let len = avoidance_slice.len().min(cohesion_slice.len()).min(alignment_slice.len());
        for (i, ((a_chunk, c_chunk), al_chunk)) in avoidance_slice.chunks(4)
    ...
    fn compute_combined_behavior_simd(&self, behaviors: &[Array1<f64>]) -> Array1<f64> {
        ...
        for behavior in &behaviors[1..] {
            ...
            for (i, chunk) in behavior_slice.chunks(4).enumerate() {
                let simd_chunk = F64x4::from_slice_unaligned(chunk);
                let simd_combined = F64x4::from_slice_unaligned(&combined_slice[i*4..(i*4+4).min(combined_slice.len())]);
    ...
    async fn compute_avoidance_behavior(&self, neighbors: &[Uuid], ctx: &AgentContext) -> Result<Array1<f64>, AgentError> {
    ...
    async fn compute_cohesion_behavior(&self, neighbors: &[Uuid], ctx: &AgentContext) -> Result<Array1<f64>, AgentError> {
    ...
    async fn compute_alignment_behavior(&self, neighbors: &[Uuid], ctx: &AgentContext) -> Result<Array1<f64>, AgentError> {
    ...
    async fn apply_behavior_weights(&self, obs: f64) -> Result<f64, AgentError> {
        ...
        let weighted_action = obs * weights.avoidance + obs * weights.cohesion + obs * weights.alignment;
        Ok(weighted_action)
    }

    async fn compute_sensor_reading(&self, neighbors: &[Uuid], ctx: &AgentContext) -> Result<f64, AgentError> {
