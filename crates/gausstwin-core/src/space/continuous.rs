...
use super::{Space, VecN, Bounds, Position, SpaceExtent};
use rand;
use rand::Rng;
...
impl ContinuousSpace {
    ...
    fn is_valid_position(&self, pos: &Vector3<f64>) -> bool {
        pos.x >= self.bounds.min.x && pos.x <= self.bounds.max.x &&
        pos.y >= self.bounds.min.y && pos.y <= self.bounds.max.y &&
        pos.z >= self.bounds.min.z && pos.z <= self.bounds.max.z
    }
...
impl Space for ContinuousSpace {
        ...
        match position {
            Position::Continuous(coords) => {
                if self.positions.contains_key(&agent_id) {
                    return Err(GaussTwinError::InvalidPosition(format!("Agent {} already exists in space", agent_id)));
                }
                if !self.is_valid_position(&coords) {
                    return Err(GaussTwinError::InvalidPosition("Position out of bounds".to_string()));
                }
                self.positions.insert(agent_id, coords);
                Ok(())
            }
            _ => Err(GaussTwinError::InvalidPosition("Expected continuous position".to_string()))
        }
    }

    fn remove_agent(&mut self, agent_id: AgentId) -> Result<()> {
        if self.positions.remove(&agent_id).is_none() {
            return Err(GaussTwinError::InvalidPosition(format!("Agent {} does not exist in space", agent_id)));
        }
        Ok(())
    }
    ...
    fn set_position(&mut self, agent_id: AgentId, position: Position) -> Result<()> {
        match position {
            ...
            _ => Err(GaussTwinError::InvalidPosition("Invalid position type for continuous space".to_string()))
        }
    }
    ...
    fn get_empty_positions(&self) -> Vec<Position> {
        ...
        for _ in 0..10 { // Sample 10 positions
            let x = rand::random::<f64>() * (self.bounds.max.x - self.bounds.min.x) + self.bounds.min.x;
            let y = rand::random::<f64>() * (self.bounds.max.y - self.bounds.min.y) + self.bounds.min.y;
            let z = rand::random::<f64>() * (self.bounds.max.z - self.bounds.min.z) + self.bounds.min.z;
    ...
    fn query_radius(&self, center: &Position, radius: f64) -> Vec<AgentId> {
        match center {
            Position::Continuous(center_coords) => {
                self.positions
                    .iter()
                    .filter(|(_, pos)| {
    ...
    fn nearest_neighbors(&self, position: &Position, k: usize) -> Vec<AgentId> {
        match position {
            Position::Continuous(pos_coords) => {
                let mut distances: Vec<_> = self.positions
                    .iter()
                    .map(|(id, pos)| {
