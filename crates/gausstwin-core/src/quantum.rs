...
impl QuantumGeneticAlgorithm {
        ...
        F: Fn(&[bool]) -> f64,
    {
        ...
        for individual in &mut self.quantum_population {
            individual.measured_solution = individual.qubits.iter()
        ...
        // Find best fitness
        let best_fitness = self.quantum_population.iter()
            .map(|ind| ind.fitness)
    ...
    pub fn best_solution(&self) -> (Vec<bool>, f64) {
        let best_individual = self.quantum_population.iter()
            .max_by(|a, b| a.fitness.partial_cmp(&b.fitness).unwrap())
            .unwrap();
        
        (best_individual.measured_solution.clone(), best_individual.fitness)
    }
...
impl QuantumParticleSwarm {
    ...
    pub fn new(
        ...
        for _ in 0..swarm_size {
            ...
            let quantum_state: Vec<f64> = (0..dimensions)
        ...
        for (i, particle) in self.particles.iter_mut().enumerate() {
            ...
            for d in 0..particle.position.len() {
                ...
                particle.velocity[d] = self.inertia_weight * particle.velocity[d]
                    + self.cognitive_coefficient * r1 * (particle_ref.best_position[d] - particle.position[d])
                    + self.social_coefficient * r2 * (self.global_best_position[d] - particle.position[d]);
    ...
    fn apply_quantum_effects(&mut self) {
        ...
        for particle in &mut self.particles {
            for d in 0..particle.position.len() {
                ...
                particle.quantum_state[d] = (particle.quantum_state[d] + rng.gen_range(-0.01..0.01)).clamp(0.0, 1.0);
            }
        }
    }
}
...
impl QuantumAnnealer {
    ...
    pub fn anneal<F>(&mut self, energy_function: F, max_iterations: usize) -> Result<(Vec<bool>, f64)>
    where
        F: Fn(&[bool]) -> f64,
    {
        let mut rng = thread_rng();
...
mod tests {
    ...
    fn test_quantum_genetic_algorithm() {
        ...
        let fitness_fn = |chromosome: &[bool]| {
            chromosome.iter().filter(|&&bit| bit).count() as f64
        };
    ...
    fn test_quantum_annealer() {
        ...
        let energy_fn = |solution: &[bool]| {
            -(solution.iter().filter(|&&bit| bit).count() as f64)
        };
