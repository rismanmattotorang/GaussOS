...
use crate::{agent::{AgentId, AgentState}, error::Result, time::SimTime};
...
impl PriorityScheduler {
    ...
    fn rebuild_queue(&mut self) {
        ...
        for (&agent_id, &priority) in &self.agent_priorities {
            self.priority_queue.push(PriorityItem { agent_id, priority });
        }
    }
}
...
impl<S: AgentState> Scheduler<S> for PriorityScheduler {
    ...
    fn add_agent(&mut self, agent_id: AgentId) -> Result<()> {
        if !self.agent_priorities.contains_key(&agent_id) {
            self.agent_priorities.insert(agent_id, 0);
            self.priority_queue.push(PriorityItem { agent_id, priority: 0 });
...
mod tests {
    ...
    fn test_random_scheduler() {
        ...
        assert_eq!(<RandomScheduler as Scheduler<TestState>>::stats(&scheduler).active_agents, 3);
        
        let mut executed_agents = Vec::new();
        while <RandomScheduler as Scheduler<TestState>>::has_next(&scheduler) {
            let batch = <RandomScheduler as Scheduler<TestState>>::next_batch(&mut scheduler, SimTime::zero()).unwrap();
    ...
    fn test_sequential_scheduler() {
        ...
        let batch1 = <SequentialScheduler as Scheduler<TestState>>::next_batch(&mut scheduler, SimTime::zero()).unwrap();
        assert_eq!(batch1.len(), 1);
        assert_eq!(batch1[0], agents[0]);
        
        let batch2 = <SequentialScheduler as Scheduler<TestState>>::next_batch(&mut scheduler, SimTime::zero()).unwrap();
        assert_eq!(batch2.len(), 1);
        assert_eq!(batch2[0], agents[1]);
        
        assert!(!<SequentialScheduler as Scheduler<TestState>>::has_next(&scheduler));
    }
    ...
    fn test_parallel_scheduler() {
        ...
        let batch1 = <ParallelScheduler as Scheduler<TestState>>::next_batch(&mut scheduler, SimTime::zero()).unwrap();
        assert_eq!(batch1.len(), 2);
        
        let batch2 = <ParallelScheduler as Scheduler<TestState>>::next_batch(&mut scheduler, SimTime::zero()).unwrap();
        assert_eq!(batch2.len(), 1);
        
        assert!(!<ParallelScheduler as Scheduler<TestState>>::has_next(&scheduler));
    }
} 