...
use crate::{
    agent::{Agent, AgentId, AgentSet, AgentContext},
    space::{Space, VecN, Bounds},
    scheduler::{Scheduler, SchedulerKind},
    time::{SimTime, TimeStep, Duration},
    event::EventQueue,
    error::Result,
};
...
pub trait Model<S: crate::agent::AgentState>: Send + Sync {
    ...
    fn run(&mut self, duration: Option<Duration>) -> impl std::future::Future<Output = Result<()>> + Send;
    ...
    fn add_agent(&mut self, agent: Box<dyn Agent<State = S>>) -> impl std::future::Future<Output = Result<AgentId>> + Send;
    
    /// Remove an agent from the model
    fn remove_agent(&mut self, id: AgentId) -> impl std::future::Future<Output = Result<()>> + Send;
    ...
    fn restore(&mut self, snapshot: serde_json::Value) -> impl std::future::Future<Output = Result<()>> + Send;
}
...
impl<S: crate::agent::AgentState + Default> Model<S> for StandardModel<S> {
    ...
    fn step(&mut self) -> impl std::future::Future<Output = Result<bool>> + Send {
        async move {
            ...
            match self.state {
                ...
                _ => return Err(GaussTwinError::ModelExecutionFailed(
                    format!("Cannot step model in state {:?}", self.state)
                )),
            }
        ...
        async move {
            ...
            if let Some(max_agents) = self.config.max_agents {
                if self.agents.len() >= max_agents {
                    return Err(crate::error::GaussTwinError::ResourceExhausted(
                        format!("Maximum number of agents ({}) reached", max_agents)
                    ));
                }
            }
    ...
    fn restore(&mut self, _snapshot: serde_json::Value) -> impl std::future::Future<Output = Result<()>> + Send {
        async move {
