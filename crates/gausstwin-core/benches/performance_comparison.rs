...
fn bench_agent_creation(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000, 100000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("mesa_equivalent", size), size, |b, &size| {
            b.iter(|| {
...
fn bench_spatial_queries(c: &mut Criterion) {
    ...
    for agent_count in [1000, 10000, 100000].iter() {
        group.bench_with_input(BenchmarkId::new("adaptive_hash_grid", agent_count), agent_count, |b, &count| {
            let mut grid = spatial::AdaptiveHashGrid::new(10.0);
            
            // Insert agents
            for i in 0..count {
                let agent_id = AgentId::from_raw(i);
                let position = space::VecN::Vec2D(
                    (i as f64 % 100.0) * 10.0,
                    ((i / 100) as f64) * 10.0,
                );
        ...
        group.bench_with_input(BenchmarkId::new("mesa_linear_search", agent_count), agent_count, |b, &count| {
            let mut agents = Vec::new();
            for i in 0..count {
...
fn bench_parallel_processing(c: &mut Criterion) {
    ...
    for agent_count in [1000, 10000, 100000].iter() {
        group.bench_with_input(BenchmarkId::new("rayon_parallel", agent_count), agent_count, |b, &count| {
            let agents: Vec<_> = (0..count).map(AgentId::from_raw).collect();
            
            b.iter(|| {
                use rayon::prelude::*;
                let results: Vec<_> = agents.par_iter()
                    .map(|&agent_id| {
        ...
        group.bench_with_input(BenchmarkId::new("sequential", agent_count), agent_count, |b, &count| {
