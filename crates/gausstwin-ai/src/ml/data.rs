...
impl DataLoader {
    ...
    pub fn next_batch(&mut self) -> Option<(Tensor, Tensor)> {
        ...
        let batch_data = self.dataset.data.narrow(0, self.current_index as i64, (end_index - self.current_index) as i64);
        let batch_labels = self.dataset.labels.narrow(0, self.current_index as i64, (end_index - self.current_index) as i64);
