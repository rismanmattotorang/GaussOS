...
pub use ml::{Model, ModelConfig, ModelState, ModelMetrics, ModelFactory};
pub use rl::{Environment, Policy, Value, Trajectory, Experience};
pub use llm::{LLMConfig};
pub use marl::{MARLConfig, MARLAgent, SyncMode, RewardStrategy};
    ...
    pub async fn train(&mut self) -> Result<()> {
        for step in 0..self.config.max_steps {
            ...
            for agent in &self.agents {
                ...
                let action = agent.act(&crate::core::State { timestamp: 0, data: state.iter().map(|&x| x as f64).collect() }).await?;
